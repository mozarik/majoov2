...
func GenerateTokensAndSetCookies(user RegisterLoginBody, c echo.Context) error {
...
func generateAccessToken(user RegisterLoginBody) (string, time.Time, error) {
...
func generateRefreshToken(user RegisterLoginBody) (string, time.Time, error) {
	// Declare the expiration time of the token
...
func generateToken(user RegisterLoginBody, expirationTime time.Time, secret []byte) (string, time.Time, error) {
...
func setUserCookie(user RegisterLoginBody, expiration time.Time, c echo.Context) {
	cookie := new(http.Cookie)
	cookie.Name = "user"
		...
		if time.Unix(claims.ExpiresAt, 0).Sub(time.Now()) < 15*time.Minute {
			...
			if err == nil && rc != nil {
				...
				if tkn != nil && tkn.Valid {
					...
					_ = GenerateTokensAndSetCookies(RegisterLoginBody{
